...
describe('Florería Rouss - Catálogo & Componentes', () => {
  ...
  it('displays official WhatsApp products like Ramos Buchones and prices in Soles', async () => {
    render(<App />)
    const catalogBtns = screen.getAllByRole('button', { name: 'Catálogo Exclusivo' })
    fireEvent.click(catalogBtns[0])

    // Verify presence of WhatsApp products
    expect(screen.getByText(/Ramo Buchón "Sol Radiante"/i)).toBeInTheDocument()
    expect(screen.getByText('S/ 180.00')).toBeInTheDocument()
    expect(screen.getByText(/Ramo Buchón Imperial 50 Rosas/i)).toBeInTheDocument()
    expect(screen.getAllByText('S/ 300.00').length).toBeGreaterThanOrEqual(1)
    expect(screen.getByText(/Maxi Ramo Buchón 80 Rosas/i)).toBeInTheDocument()
    expect(screen.getByText('S/ 450.00')).toBeInTheDocument()
  })

  it('filters products accurately when typing in search input', async () => {
    render(<App />)
    const catalogBtns = screen.getAllByRole('button', { name: 'Catálogo Exclusivo' })
    fireEvent.click(catalogBtns[0])

    const searchInput = screen.getByPlaceholderText(/Buscar arreglo/i)
    fireEvent.change(searchInput, { target: { value: 'Girasoles' } })

    expect(screen.getByText(/Ramo Buchón "Sol Radiante" \(12 Girasoles\)/i)).toBeInTheDocument()
    expect(screen.getByText(/Maxi Ramo Buchón 80 Rosas & 6 Girasoles/i)).toBeInTheDocument()
  })
