...
- **Optimización de Imágenes**: Convertidor de imágenes de alta fidelidad **WebP** (`imageOptimizer.js` y `convert-to-webp.js`) 100% compatible con **iOS (Safari iOS 14+)**, **Android** y navegadores de escritorio.
...
Se cargaron los **14 arreglos florales oficiales** enviados directamente por el dueño vía WhatsApp, estructurados con descripciones profesionales de alta gama sin emojis informales:

| # | Título Oficial del Arreglo | Categoría | Precio (S/) | Formato de Imagen |
| :- | :--- | :--- | :--- | :--- |
| 1 | **Ramo Buchón "Sol Radiante" (12 Girasoles)** | Ramos Buchones & Girasoles | S/ 180.00 | WebP + JPG Fallback |
| 2 | **Ramo Buchón Imperial 50 Rosas & Trío de Helio** | Ramos Buchones & Girasoles | S/ 300.00 | WebP + JPG Fallback |
| 3 | **Maxi Ramo Buchón 80 Rosas & 6 Girasoles** | Ramos Buchones & Girasoles | S/ 450.00 | WebP + JPG Fallback |
| 4 | **Ramo Buchón "Reina Imperial" (80 Rosas)** | Ramos Buchones & Girasoles | S/ 400.00 | WebP + JPG Fallback |
| 5 | **Ramo Buchón "Corona de Reina" (50 Rosas)** | Cumpleaños & Coronales | S/ 300.00 | WebP + JPG Fallback |
| 6 | **Ramo Buchón Élite 50 Rosas** | Ramos Buchones & Girasoles | S/ 250.00 | WebP + JPG Fallback |
| 7 | **Ramo Buchón 40 Rosas & Mariposas de Oro** | Ramos Buchones & Girasoles | S/ 200.00 | WebP + JPG Fallback |
| 8 | **Ramo Buchón Pasión 30 Rosas** | Rosas Premium | S/ 170.00 | WebP + JPG Fallback |
| 9 | **Combo Ternura 20 Rosas, Peluche & Lirio** | Detalles & Combos Especiales | S/ 230.00 | WebP + JPG Fallback |
| 10 | **Bouquet Conservación 20 Rosas & Hidratación** | Rosas Premium | S/ 150.00 | WebP + JPG Fallback |
| 11 | **Bouquet Armonía 12 Rosas & Astromelias** | Rosas Premium | S/ 160.00 | WebP + JPG Fallback |
| 12 | **Bouquet Esencial 15 Rosas Hidratadas** | Rosas Premium | S/ 100.00 | WebP + JPG Fallback |
| 13 | **Bouquet Imperial de Lirios Blancos** | Delicadeza Pastel & Lirios | S/ 150.00 | WebP + JPG Fallback |
| 14 | **Bouquet Romance 3 Lirios & Rosas Pastel** | Delicadeza Pastel & Lirios | S/ 160.00 | WebP + JPG Fallback |

---

## 3. 🖼️ Módulo Convertidor y Optimizador WebP

1. **Script CLI Automatizado (`scripts/convert-to-webp.js`)**:
   - `npm run optimize:images`: Procesa todas las imágenes de la carpeta `Productos Rouss/`, escala a un máximo de 1200px (calidad 88%), preserva el perfil de color sRGB y genera tanto `.webp` como `.jpg` para compatibilidad total.
...
- **Vitest**: `npm test` ejecuta 7 pruebas unitarias verificando:
  ...
  - Búsqueda y filtrado instantáneo en vivo.
...
| **`--whatsapp-solid`** | `#25D366` | Verde oficial WhatsApp sólido (sin degradados ni brillos). |
...
## 6. ☁️ Infraestructura en la Nube y Base de Datos
...
- **Enrutamiento SPA**: [`vercel.json`](file:///c:/Users/JACK%20FRANKLIN/Downloads/Floreria%20rouss/vercel.json) con rewrites automáticos.
- **Supabase Cloud (`llasbukvdjlvwlgofgke` - sa-east-1)**:
  - Tenant oficial: `a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11` (`slug: 'rouss'`).
  - Tablas activas: `tenants`, `tenant_settings`, `banners`, `categories`, `products`, `testimonios`, `reservations`.
  - Seguridad RLS: Habilitada en todas las tablas con políticas de lectura pública y protección estricta de pedidos.
